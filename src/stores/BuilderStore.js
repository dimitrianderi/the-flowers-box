...
const useBuilderStore = defineStore('builderStore', () => {
  ...
  const costFlowers = computed(() => bouquet.flowers
    .reduce((total, flower) => total + +flower.res, 0)
    .toFixed(2));

  const costPackagings = computed(() => (
    bouquet.packaging.length ? bouquet.packaging[0].price : 0
  ));
  const costGreeneries = computed(() => (
    bouquet.greenery.length ? bouquet.greenery[0].price * +totalFlowers.value : 0
  ));
  const costComposition = computed(() => (
    bouquet.composition.length ? bouquet.composition[0].price : 1
  ));

  const getResultCost = computed(() => ((
    +costFlowers.value + +costPackagings.value + +costGreeneries.value) * +costComposition.value)
    .toFixed(2));
  ...
  const addGreenery = (data) => {
    bouquet.greenery = [{
      title: data.title,
      price: data.price,
      amount: computed(() => totalFlowers.value),
      res: computed(() => (+totalFlowers.value * data.price).toFixed(2)),
    }];
  };
  ...
  const submitHandler = async (name) => {
    ...
    bouquet.greenery = [{
      ...bouquet.greenery[0],
      amount: totalFlowers.value,
      res: (+totalFlowers.value * bouquet.greenery[0].price).toFixed(2),
    }];
